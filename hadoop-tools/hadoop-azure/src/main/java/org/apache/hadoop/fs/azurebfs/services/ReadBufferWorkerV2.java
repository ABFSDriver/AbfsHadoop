...
class ReadBufferWorkerV2 implements Runnable {

  protected static final CountDownLatch UNLEASH_WORKERS = new CountDownLatch(1);
  private int id;
  private AtomicBoolean isRunning = new AtomicBoolean(true);

  ReadBufferWorkerV2(final int id) {
    this.id = id;
  }

  /**
   * return the ID of ReadBufferWorkerV2.
   */
  public int getId() {
    return this.id;
  }
  ...
  public void run() {
    ...
    ReadBufferManagerV4 bufferManager = ReadBufferManagerV4.getBufferManager();
    ReadBuffer buffer;
    while (isRunning()) {
      try {
