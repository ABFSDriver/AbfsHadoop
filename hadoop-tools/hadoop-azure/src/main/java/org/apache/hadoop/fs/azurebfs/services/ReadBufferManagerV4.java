 ...
 */
public final class ReadBufferManagerV4 extends ReadBufferManager {
  ...
  void init() {
    ...
    memoryMonitorThread = Executors.newSingleThreadScheduledExecutor(
        runnable -> {
          Thread t = new Thread(runnable, "ReadAheadV4-Memory-Monitor");
          t.setDaemon(true);
          return t;
        });
    ...
    ReadBufferWorkerV2.UNLEASH_WORKERS.countDown();

    printTraceLog(
        "ReadBufferManagerV4 initialized with {} buffers and {} worker threads",
   ...
   * from {@link ReadBufferManagerV4} when stream is closed.
  ...
  private static void setBufferManager(ReadBufferManagerV4 manager) {
  ...
  @VisibleForTesting
  synchronized static ReadBufferManagerV4 getInstance() {
    return bufferManager;
  }
